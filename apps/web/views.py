...
def current_term(request, sort):
    ...
    term_courses = Course.objects.for_term(
        constants.CURRENT_TERM, dist).prefetch_related(
            'distribs', 'review_set', 'courseoffering_set').order_by(
...
def course_detail(request, course_id):
    ...
    professors_and_review_count += [
        (instructor_name, 0) for instructor_name in Instructor.objects.filter(
            courseoffering__course=course, ).exclude(
                name__in=[
    ...
    return JsonResponse({
        ...
        sorted(
            [{
                ...
                'courses':
                term_medians,
            } for term, term_medians in list(medians_by_term.items())],
