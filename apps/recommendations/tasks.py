...
def generate_course_description_similarity_recommendations():
    ...
    course_descriptions = []
    i = 0
    for course in Course.objects.exclude(description=None).exclude(
            description=""):
    ...
    course_ids_to_zero = Course.objects.filter(
        Q(title__icontains="thesis") | Q(title__icontains="research")
        | Q(title__icontains="independent") | Q(title__icontains="seminar")
        | Q(title__icontains="first-year")
        | Q(title__icontains="foreign study") | Q(title__icontains="senior")
        | Q(title__icontains="honors") | Q(number__gt=99)).values_list(
            "id", flat=True)
    ...
    for i in range(psarray.shape[1]):
        ...
        for xlist_course in list(
                chain(course.crosslisted_courses.all(),
    ...
    for i in range(psarray.shape[0]):
        ...
        zero_ids += Course.objects.filter(
            title=current_class.title).values_list(
                "id", flat=True)
