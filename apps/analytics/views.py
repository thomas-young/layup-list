...
def home(request):
    ...
    for name, earliest_date in [month_ago, week_ago, today]:
        vote_table.append((
            ...
            models.Vote.objects.filter(
                ...
                category=models.Vote.CATEGORIES.DIFFICULTY,
                created_at__gte=earliest_date).count(),
            models.Vote.objects.filter(value=0,
                                       created_at__gte=earliest_date).count(),
        ))
    ...
    class_breakdown = sorted([(
        year,
        count,
    ) for year, count in list(c.items()) if len(year) == 2])
    ...
    for creator, description in Recommendation.CREATORS:
        ...
        if rec:
            recommendations_last_updated.append((description,
                                                 rec[0].created_at))
        else:
...
def eligible_for_recommendations(request):
    eligible_users_and_votes = (models.Vote.objects.filter(
        value=1,
        category=models.Vote.CATEGORIES.QUALITY).values_list('user').annotate(
            vote_count=Count('user')).filter(
                vote_count__gte=constants.REC_UPVOTE_REQ).order_by(
                    '-vote_count').values_list('user__username', 'user',
                                               'vote_count'))
...
def sentiment_labeler(request):
    ...
    return render(
        request, 'sentiment_labeler.html', {
            ...
            count,
            'labeled_count':
            models.Review.objects.filter(sentiment_labeler=models.Review.
                                         MANUAL_SENTIMENT_LABELER).count(),
            'form':
            form,
            'review':
            review,
